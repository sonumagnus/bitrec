...
class WidgetCategory {
  ...
  WidgetCategory([this.categoryName, this.controlList, this.mobileCategoryId, this.webCategoryId, this.platformsToHide]);

  /// Getting the control details from the json file
  factory WidgetCategory.fromJson(Map<String, dynamic> json) {
    return WidgetCategory(json['categoryName'], json['controlList'], json['mobileCategoryId'], json['webCategoryId'], json['platformsToHide']);
  }
...
class Control {
  ...
  Control(this.title, this.description, this.image, this.status, this.displayType, this.subItems, this.controlId, this.isBeta, this.platformsToHide);

  /// Getting the control details from the json file
  factory Control.fromJson(Map<String, dynamic> json) {
    return Control(json['title'], json['description'], json['image'], json['status'], json['displayType'], json['subItems'], json['controlId'],
        json['isBeta'], json['platformsToHide']);
  }
...
class SubItem {
  ...
  factory SubItem.fromJson(Map<String, dynamic> json) {
    return SubItem(json['type'], json['displayType'], json['title'], json['key'], json['codeLink'], json['description'], json['status'],
        json['subItems'], json['sourceLink'], json['sourceText'], json['needsPropertyPanel'], json['platformsToHide']);
  }
...
class SampleModel extends Listenable {
  ...
  final Map<String, Function> sampleWidget = {'aa': () {}};
...
Future<void> updateControlItems() async {
  ...
  for (int index = 0; index < categoryList.length; index++) {
    ...
    if (SampleModel._categoryList[index].platformsToHide == null || _needToShow(SampleModel._categoryList[index].platformsToHide)) {
      for (int i = 0; i < SampleModel._categoryList[index].controlList!.length; i++) {
        ...
        if (controlList[i].platformsToHide == null || _needToShow(controlList[i].platformsToHide)) {
          for (int j = 0; j < controlList[i].subItems!.length; j++) {
            firstLevelSubItems.add(SubItem.fromJson(controlList[i].subItems![j]));
            if (firstLevelSubItems[j].type == 'parent') {
              for (int k = 0; k < firstLevelSubItems[j].subItems!.length; k++) {
                if (SubItem.fromJson(firstLevelSubItems[j].subItems![k]).platformsToHide == null ||
                    _needToShow(SubItem.fromJson(firstLevelSubItems[j].subItems![k]).platformsToHide)) {
                  secondLevelSubItems.add(SubItem.fromJson(firstLevelSubItems[j].subItems![k]));
                  for (int l = 0; l < secondLevelSubItems[secondLevelSubItems.length - 1].subItems!.length; l++) {
                    if (SubItem.fromJson(secondLevelSubItems[secondLevelSubItems.length - 1].subItems![l]).platformsToHide == null ||
                        _needToShow(SubItem.fromJson(secondLevelSubItems[secondLevelSubItems.length - 1].subItems![l]).platformsToHide)) {
                      thirdLevelSubItems.add(SubItem.fromJson(secondLevelSubItems[secondLevelSubItems.length - 1].subItems![l]));
                    }
                    thirdLevelSubItems[thirdLevelSubItems.length - 1].parentIndex = j;
                    thirdLevelSubItems[thirdLevelSubItems.length - 1].childIndex = k;
                    thirdLevelSubItems[thirdLevelSubItems.length - 1].sampleIndex ??= thirdLevelSubItems.length - 1;
                    thirdLevelSubItems[thirdLevelSubItems.length - 1].control = controlList[i];
                    final String breadCrumbText =
                        ('/${controlList[i].title!}/${firstLevelSubItems[j].title!}/${secondLevelSubItems[secondLevelSubItems.length - 1].title!}${secondLevelSubItems[secondLevelSubItems.length - 1].subItems!.length == 1 ? '' : ('/${thirdLevelSubItems[thirdLevelSubItems.length - 1].title!}')}')
                            .replaceAll(' ', '-')
                            .toLowerCase();
                    thirdLevelSubItems[thirdLevelSubItems.length - 1].breadCrumbText = breadCrumbText;
                    thirdLevelSubItems[thirdLevelSubItems.length - 1].categoryName = SampleModel._categoryList[index].categoryName;
                    sampleRoutes.add(SampleRoute(routeName: breadCrumbText, subItem: thirdLevelSubItems[thirdLevelSubItems.length - 1]));
                  }
                  secondLevelSubItems[secondLevelSubItems.length - 1].subItems = thirdLevelSubItems;
              ...
              if (firstLevelSubItems[j].platformsToHide == null || _needToShow(firstLevelSubItems[j].platformsToHide)) {
                isChild = true;
                for (int k = 0; k < firstLevelSubItems[j].subItems!.length; k++) {
                  if (SubItem.fromJson(firstLevelSubItems[j].subItems![k]).platformsToHide == null ||
                      _needToShow(SubItem.fromJson(firstLevelSubItems[j].subItems![k]).platformsToHide)) {
                    secondLevelSubItems.add(SubItem.fromJson(firstLevelSubItems[j].subItems![k]));
                    secondLevelSubItems[secondLevelSubItems.length - 1].childIndex = j;
                    secondLevelSubItems[secondLevelSubItems.length - 1].sampleIndex ??= k;
                    secondLevelSubItems[secondLevelSubItems.length - 1].control = controlList[i];
                    String breadCrumbText;
                    if (firstLevelSubItems[j].subItems!.length == 1 && secondLevelSubItems.length == 1) {
                      breadCrumbText = ('/${controlList[i].title!}/${secondLevelSubItems[secondLevelSubItems.length - 1].title!}')
                          .replaceAll(' ', '-')
                          .toLowerCase();
                    } else {
                      breadCrumbText =
                    ...
                    secondLevelSubItems[secondLevelSubItems.length - 1].breadCrumbText = breadCrumbText;
                    secondLevelSubItems[secondLevelSubItems.length - 1].categoryName = SampleModel._categoryList[index].categoryName;
                    sampleRoutes.add(SampleRoute(routeName: breadCrumbText, subItem: secondLevelSubItems[secondLevelSubItems.length - 1]));
                  }
                }
                firstLevelSubItems[j].subItems = secondLevelSubItems;
                secondLevelSubItems = <SubItem>[];
              ...
              if (firstLevelSubItems[j].platformsToHide == null || _needToShow(firstLevelSubItems[j].platformsToHide)) {
                final String breadCrumbText = ('/${controlList[i].title!}/${firstLevelSubItems[j].title!}').replaceAll(' ', '-').toLowerCase();
                firstLevelSubItems[j].breadCrumbText = breadCrumbText;
                firstLevelSubItems[j].control = controlList[i];
                firstLevelSubItems[j].categoryName = SampleModel._categoryList[index].categoryName;
                sampleRoutes.add(SampleRoute(routeName: breadCrumbText, subItem: firstLevelSubItems[j]));
          ...
          (!isSample) ? controlList[i].subItems = firstLevelSubItems : isSample = false;
  ...
  if (isWeb) {
    ...
    SampleModel._categoryList.sort((WidgetCategory a, WidgetCategory b) => a.mobileCategoryId!.compareTo(b.mobileCategoryId!));
  }
}
...
class SampleRoute {
  ...
  SampleRoute({this.routeName, this.subItem, this.currentContext, this.currentState, this.currentWidget, this.globalKey});
